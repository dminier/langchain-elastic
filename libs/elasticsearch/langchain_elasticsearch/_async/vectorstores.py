...
from elasticsearch.helpers.vectorstore import AsyncVectorStore as EVectorStore
...
def _convert_retrieval_strategy(
    ...
    if isinstance(langchain_strategy, ApproxRetrievalStrategy):
        ...
        return AsyncDenseVectorStrategy(
        ...
        return AsyncDenseVectorScriptScoreStrategy(
            distance=DistanceMetric[distance.value]
        )
    elif isinstance(langchain_strategy, SparseRetrievalStrategy):
        return AsyncSparseVectorStrategy(langchain_strategy.model_id)
    elif isinstance(langchain_strategy, BM25RetrievalStrategy):
        return AsyncBM25Strategy(k1=langchain_strategy.k1, b=langchain_strategy.b)
    else:
        raise TypeError(
...
class AsyncElasticsearchStore(VectorStore):
    ...
    **Advanced Uses:**
    ...
    def __init__(
        ...
        *,
        ...
        if embedding:
            embedding_service = AsyncEmbeddingServiceAdapter(embedding)
    ...
    def connect_to_elasticsearch(
        *,
        ...
        return create_async_elasticsearch_client(
            url=es_url,
            cloud_id=cloud_id,
            api_key=api_key,
            username=username,
    ...
    async def asimilarity_search_with_score(
        ...
        **kwargs: Any,
        ...
        """
        if (
            isinstance(self._store.retrieval_strategy, AsyncDenseVectorStrategy)
